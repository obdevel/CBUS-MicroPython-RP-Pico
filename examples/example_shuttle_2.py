...
class mymodule(cbusmodule.cbusmodule):
    ...
    def initialise(self):
        ...
        self.module_id = 105
        self.module_name = bytes('PYSHTL2', 'ascii')
        self.module_params = [
            20,
            cbusdefs.MANU_MERG,
            0,
    ...
    async def shuttle(self, decoder_id: int, speed: int=10, delay: int=10000) -> None:
        ...
        try:
            ...
            while True:
                ...
                s = await WaitAny((self.sensor1, self.sensor2, self.sensor3)).wait()
                self.logger.log(f'** {s.name} has changed state to {s.state}')

                if s.state == cbusobjects.OBJECT_STATE_ON:
    ...
    async def run(self) -> None:
        ...
        self.ts = asyncio.create_task(self.shuttle(27, 10, 10000))
